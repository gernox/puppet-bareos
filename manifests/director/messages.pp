# @summary
#   Manages the bareos messages configuration
#
class gernox_bareos::director::messages {
  ...
  ::bareos::director::messages { 'Standard':
    ...
    operator_command =>
      '/usr/bin/bsmtp -h localhost -f \"\(Bareos\) \<%r\>\" -s \"Bareos: Intervention needed for %j\" %r',
